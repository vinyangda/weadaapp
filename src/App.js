...
function App() {
  ...
  const API_KEY="d61cf641bb98e61451beaace9c15aa17"; // API키 선언
  const [location, setLocation] = useState('');     // 검색을 했을 때, 받아올 스테이트  
  const [result, setResult] = useState({});         // 도시, 온도, 상태를 확인받을 스테이트 
  
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${API_KEY}`; //키를 url로 사용하고 싶을 때
